...
test_that("LinRegRC rejects errounous input", {
  expect_error(linreg_mod <- LinRegRC$new(formula = Petal.Length~Sepdsal.Width+Sepal.Length, data=iris))
  expect_error(linreg_mod <- LinRegRC$new(formula = Petal.Length~Sepdsal.Width+Sepal.Length, data=irfsfdis))
})
...
test_that("class is correct", {
  ...
  expect_true(class(linreg_mod)[1] == "LinRegRC")
})
